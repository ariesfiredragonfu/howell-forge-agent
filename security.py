...
def check_https_redirect():
    ...
    class NoRedirect(urllib.request.HTTPRedirectHandler):
        def redirect_request(self, req, fp, code, msg, hdrs, newurl):
            return None  # Don't follow — raises HTTPError with redirect info
...
def main() -> int:
    ...
    biofeedback.append_constraint("SECURITY", f"Security check failed — {full_msg}")
    telegram_msg = f"🛡️ [{severity}] Security: {full_msg}"
    send_telegram_alert(telegram_msg)
    print(f"Security: ALERT — {full_msg} (wrote to log, Telegram sent)")
    return 1
